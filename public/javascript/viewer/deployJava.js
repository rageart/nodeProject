var deployJava = function() {
	...
	var d = {
		...
		getAppletTag : function(r, w) {
			var o = "<" + "applet MAYSCRIPT=\"MAYSCRIPT\"";
			var q = "";
			var t = "<" + "/" + "applet" + ">";
			var x = true;
...
//				o += ' data-' + u.replace(/[A-Z]/, '-$&').toLowerCase()  + '="' + w[u] + '"';
			}
			q += '<param name="java_arguments" value="-Xms256m -Xmx512m -XX:MaxPermSize=256M"/>'
			if (!v) {
					...
					if (navigator.mimeTypes[p].type == this.mimeType) {
						if (navigator.mimeTypes[p].enabledPlugin) {
							document.write("<"
									...
									+ this.mimeType + '" hidden="true" style="display:none" />');
							o = true
						}
					}
				}
